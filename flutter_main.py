...
def main_program():
    ...
    for idx_file in range(len(analysis_files)):
        ...
        for idx_range in range(len(time_ranges)):
            ...
            result_test_point = analyse_data_acc(acc_data, time_ranges, idx_range, airspeed[idx_range], altitude[idx_range], subtitle[idx_range])
...
def main_temp():
    ...
    analysis_files = cfg_analysis.CSV_FILE_ATMOS
    idx_file = 0

    print(f"Running on {analysis_files[idx_file]}...")
    atmos = import_data_atmos(analysis_files, idx_file)
